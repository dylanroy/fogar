...
- [x] Cloudflare Web Analytics runs on the web app: the zone injects its beacon into every HTML response, and rather than switch that off the app's script policy admits it (2026-09-25). Cookie-free; the network ledger shows cloudflareinsights.com, and Settings says so. The extension stays analytics-free.
...
- [x] iOS 26: wllama's compatibility build (for browsers without JSPI or Memory64) shipped from app.fogar.ai (`web/wllama/`, from `@wllama/wllama-compat` at the pinned version), picked by the worker rewrite when wllama asks for it; 15 MB, fetched at the first load, then cached. Slower than the normal build, which iOS 27 uses. Before this a load on iOS 26 hung at “Loading…” (found on the phone and on the iOS 26 simulator, 2026-09-25). `npm run test:web` forces the path in Chromium by hiding the feature wllama checks.
- [x] The header row (Customize, Focus, Device/Cloud) wraps on a phone. It used to be a few pixels wider than the page, and the grid stretched every section to match, so the page could be pulled sideways; worst in focus mode, where the button reads “Show everything” (2026-09-25).
...
- [x] Eight more use-case pages, one per feature: `/ask` (private AI), `/search` (answers with sources), `/files` (PDFs and pages), `/models` (your own key), `/draw` (Canvas), `/notebook` (Notebook, Post-its, Notes), `/news` (Feed), `/work` (Inbox and Jira). The Use cases menu is three columns (Ask and write, Organize, Connect) that fold to two and then one on narrow screens and scroll when taller than the window; the footer and a “More” row on the homepage list them (2026-09-24).
- [ ] Publish those eight when v0.1.1 is live on the store: they describe attachments and the widgets that arrive with it, and Add to Chrome installs v0.1.0 until then. Empty `HOLD` in `scripts/build-site.mjs`, `npm run site`, `npx wrangler deploy`. The same gap applies to `/voice`, already live: the Writing widget is also new in v0.1.1.
