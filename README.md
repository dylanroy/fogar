...
- Sessions widget: save your open windows and close them without fear. Per window or all at once, duplicates collapsed, searchable from the widget and the ask bar, restore with tabs loading only when clicked, promote tabs or a whole session into a bookmark folder. Needs the `tabs` permission, asked for only when the widget is added.
