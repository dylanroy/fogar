...
Local mode makes no network requests after the model is cached. Cloud mode sends prompts only to the endpoint the user typed in. No analytics in the extension. This is also the cheapest possible Chrome Web Store data disclosure. The web app fetches its own files from app.fogar.ai, and carries Cloudflare's cookie-free page-view beacon, the zone's Web Analytics, which the network ledger shows as cloudflareinsights.com; its data stays in that browser. The extension has no analytics of any kind.
